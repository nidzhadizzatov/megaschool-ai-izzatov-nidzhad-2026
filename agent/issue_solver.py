    ...
    def solve_issue(self, issue_number: int, doc_id: int = None) -> Optional[int]:
        ...
        try:
            ...
            for filepath in files:
                ...
                # Проверка на количество задач перед делением
                total = result.total_tasks if hasattr(result, 'total_tasks') else 0
                done = result.completed_tasks if hasattr(result, 'completed_tasks') else 0
